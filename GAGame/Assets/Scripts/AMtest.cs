...
public class AMtest : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i=25; i<50; i++){
            geneObject[i] = Instantiate(element, new Vector3 (0, 320, (i-25)*15), Quaternion.identity) as GameObject;
            geneObject[i].GetComponent<AMGroup> ().setScore (score[i]);
            geneObject[i].GetComponent<AMGroup> ().setColor (colorarray [i]);
        }
    ...
    private IEnumerator cross ()
    {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            yield return StartCoroutine (father.GetComponent<AMGroup> ().move (new Vector3 (0, 0, -45), 1f));
            yield return StartCoroutine (mother.GetComponent<AMGroup> ().move (new Vector3 (0, 0, -15), 1f));
            GameObject go1 = father.GetComponent<AMGroup> ().getSegment (0, r);
            GameObject go2 = mother.GetComponent<AMGroup> ().getSegment (r, 30 - 1);
            yield return StartCoroutine (go1.GetComponent<AMGenePieces> ().move (new Vector3 (0, 0, -30), 1f));
            yield return StartCoroutine (go2.GetComponent<AMGenePieces> ().move (new Vector3 (0, 7 + r * 10, -30), 1f));
